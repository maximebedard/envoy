...
envoy::config::filter::network::memcached_proxy::v2::MemcachedProxy::ConnPoolSettings
createConnPoolSettings() {
  envoy::config::filter::network::memcached_proxy::v2::MemcachedProxy::ConnPoolSettings setting{};
  setting.mutable_op_timeout()->CopyFrom(Protobuf::util::TimeUtil::MillisecondsToDuration(20));
  return setting;
}

MessageImpl getRequest(const std::string& key) {
  return MessageImpl(Message::Type::V1_REQUEST, Message::OpCode::GET, 0, 0, 0, 0, key, "", "");
}

MessagePtr getResponse(const std::string& key) {
 return std::make_unique<MessageImpl>(Message::Type::V1_RESPONSE, Message::OpCode::GET, 0, 0, 0, 0, key, "", "");
}
...
TEST_F(RedisClientImplTest, Basic) {
  ...
  auto request1 = getRequest("foo");
  MockPoolCallbacks callbacks1;
  EXPECT_CALL(*encoder_, encodeMessage(Ref(request1), _));
  PoolRequest* handle1 = client_->makeRequest(request1, callbacks1);
  EXPECT_NE(nullptr, handle1);

  onConnected();

  auto request2 = getRequest("bar");
  MockPoolCallbacks callbacks2;
  EXPECT_CALL(*encoder_, encodeMessage(Ref(request2), _));
  ...
  EXPECT_CALL(*decoder_, onData(Ref(fake_data))).WillOnce(Invoke([&](Buffer::Instance&) -> void {
    InSequence s;
    auto response1 = getResponse("foo");
    ...
    callbacks_->decodeMessage(std::move(response1));

    auto response2 = getResponse("bar");
    EXPECT_CALL(callbacks2, onResponse_(Ref(response2)));
    EXPECT_CALL(*connect_or_op_timer_, disableTimer());
    EXPECT_CALL(host_->outlier_detector_, putResult(Upstream::Outlier::Result::SUCCESS));
    callbacks_->decodeMessage(std::move(response2));
  }));
...
public:
  ...
  // MemcachedProxy::ConnPool::ClientFactory
...
TEST_F(RedisConnPoolImplTest, Basic) {
  ...
  EXPECT_CALL(*client, makeRequest(Ref(request1), Ref(callbacks))).WillOnce(Return(&active_request));
  PoolRequest* request = conn_pool_->makeRequest("hash_key", request1, callbacks);
  EXPECT_EQ(&active_request, request);
...
TEST_F(RedisConnPoolImplTest, NoClusterAtConstruction) {
  ...
  request = conn_pool_->makeRequest("hash_key", request1, callbacks);
  EXPECT_EQ(nullptr, request);
...
TEST_F(RedisConnPoolImplTest, RemoteClose) {
  ...
  EXPECT_CALL(*this, create_(_)).WillOnce(Return(client));
  EXPECT_CALL(*client, makeRequest(Ref(request1), Ref(callbacks))).WillOnce(Return(&active_request));
  conn_pool_->makeRequest("hash_key", request1, callbacks);
