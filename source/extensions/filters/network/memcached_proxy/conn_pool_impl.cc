...
InstanceImpl::ThreadLocalPool::~ThreadLocalPool() {
  ...
  while (!client_map_.empty()) {
    client_map_.begin()->second->memcached_client_->close();
  }
}
...
void InstanceImpl::ThreadLocalPool::onHostsRemoved(
  ...
  for (const auto& host : hosts_removed) {
    ...
    if (it != client_map_.end()) {
      ...
      it->second->memcached_client_->close();
    }
  }
}
...
PoolRequest* InstanceImpl::ThreadLocalPool::makeRequest(const std::string& key,
  ...
  if (!client) {
    ...
    client->host_ = host;
    client->memcached_client_ = parent_.client_factory_.create(host, dispatcher_, parent_.config_);
    client->memcached_client_->addConnectionCallbacks(*client);
  }

  return client->memcached_client_->makeRequest(request, callbacks);
}
...
void InstanceImpl::ThreadLocalActiveClient::onEvent(Network::ConnectionEvent event) {
  if (event == Network::ConnectionEvent::RemoteClose ||
    ...
    parent_.dispatcher_.deferredDelete(std::move(client_to_delete->second->memcached_client_));
